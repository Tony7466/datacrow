...
public class ImdbMovieSearch extends ImdbSearch {
    ...
    private static Logger logger = DcLogManager.getLogger(ImdbMovieSearch.class.getName());
    ...
    public DcObject getItem(URL url, boolean full, boolean alternativeTitle) throws Exception {
        ...
        try {
            Document document = null; //HtmlUtils.getDocument(url, StandardCharsets.UTF_8, 0);
    ...
    private void setActors(String movieId, DcObject movie) throws Exception {
        Document document = HtmlUtils.getDocument(new URL(getAddress() + "/title/tt" + movieId + "/fullcredits#cast"), StandardCharsets.UTF_8);
