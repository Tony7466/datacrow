...
package org.datacrow.onlinesearch.isbndb.task;
...
/*
 * 
 * TODO: reimplement using JSoup
 */

public class ISBNdbBookSearch extends SearchTask {

    private static Logger logger = DcLogManager.getLogger(ISBNdbBookSearch.class.getName());
    ...
    protected DcObject getItem(URL url, boolean full) throws Exception {
        ...
        Document document = HtmlUtils.getDocument(url, StandardCharsets.UTF_8);
    ...
    private void setTitle(DcObject book, Document document) throws Exception {
        ...
        book.setValue(Book._A_TITLE, element.text());
    }
    ...
    private void setDescription(DcObject book, Document document) throws Exception {
        ...
        String description = null;//getFollowingText(node);
        if (description != null) {
    ...
    private void retrieveAmazonDetails(DcObject book) {
        ...
        if (ASIN != null) {
            OnlineServices os = Servers.getInstance().getOnlineServices(book.getModule().getIndex());
    ...
    private void setDetails(DcObject book, Document document) throws Exception {
        ...
        if (!CoreUtilities.isEmpty(binding)) {
            ...
            if (!binding.equalsIgnoreCase("Unknown Binding")) {
                ...
                if (binding.indexOf("pbk") > -1 || binding.indexOf("paperback") > -1)
                	book.createReference(Book._U_BINDING, "Paperback");
                else if (binding.indexOf("cover") > -1)
                	book.createReference(Book._U_BINDING, "Hardcover");
            }
        }
    }
    ...
    private void setPublisher(DcObject book, Document document) throws Exception {
        ...
        if (node != null)
        	book.createReference(Book._F_PUBLISHER, node.getTextContent());
    }
    ...
    protected Collection<Object> getItemKeys() throws Exception {
        ...
        String link = getServer().getUrl() +  "/search/all?query=" + getQuery();
        Document document = HtmlUtils.getDocument(new URL(link), StandardCharsets.UTF_8);

        String expression = "//a[starts-with(@href,'http://isbndb.com/book/')]/@href";
