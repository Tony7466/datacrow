...
import net.datacrow.core.http.HttpConnectionException;
...
public class MusicBrainzMusicSearch extends MusicBrainzSearch {

    private static Logger logger = DcLogManager.getLogger(MusicBrainzMusicSearch.class.getName());
    
    public MusicBrainzMusicSearch(IOnlineSearchClient listener, 
                             IServer server, 
                             Region region, 
                             SearchMode mode,
                             String query) {
	...
	protected DcObject getItem(URL url, boolean full) throws Exception {
	    ...
	    String xml = "";
	    try {
	        xml = HttpConnectionUtil.retrievePage(url);
	...
	private void setWebpage(DcObject dco, String link) {
		dco.setValue(MusicAlbum._N_WEBPAGE, link);
	}
	
	@SuppressWarnings("unchecked")
    private void addCovers(DcObject dco) {
        OnlineServices os = Servers.getInstance().getOnlineServices(dco.getModuleIdx());
...
//        int titleIdx = MusicTrack._A_TITLE; 
//        int nrIdx = MusicTrack._F_TRACKNUMBER;
//        int lengthIdx = MusicTrack._J_PLAYLENGTH;
//        int artistIdx = MusicTrack._G_ARTIST;
        ...
        for (String s : c) {
            ...
            track.setValue(MusicTrack._A_TITLE, title);
            track.setValue(MusicTrack._F_TRACKNUMBER, Long.valueOf(number));
            ...
            try {
                ...
                if (duration.length() > 0) {
                    ...
                    track.setValue(MusicTrack._J_PLAYLENGTH, Long.valueOf(seconds));
                }
            } catch (Exception e) {
            ...
            for (DcObject artist : artists)
            	track.addMapping(artist, MusicTrack._G_ARTIST);
    ...
    private Collection<DcObject> setArtists(DcObject dco, String part) {
        ...
        for (String artist : artists) {
            try {
                ...
                int module = DcModules.getReferencedModule(dco.getField(MusicAlbum._F_ARTISTS)).getIndex();
                
                Connector connector = DcConfig.getInstance().getConnector();
                DcObject person = connector.getItemByExternalID(module, DcRepository.ExternalReferences._MUSICBRAINZ, id);
                ...
                result.add(person);
                dco.addMapping(person, MusicAlbum._F_ARTISTS);
            } catch (Exception e) {
