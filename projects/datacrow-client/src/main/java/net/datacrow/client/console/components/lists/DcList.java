...
public class DcList extends JList<Object> implements ComponentListener {
    ...
    static {
        ...
        if(!ui.contains(uiClassID))
            ui.put(uiClassID, "net.datacrow.client.console.components.lists.DcListUI");
    }
    
    public DcList(DcListModel<Object> model) {
        super(model);
    ...
    @Override
    public void setListData(Vector<? extends Object> v) {
        addElements(v);
    }
    
    public void addElements(List<? extends Object> elements) {
        getDcModel().setSize(0);
        for (Object element : elements)
            getDcModel().addElement(element);
    }
    
    public DcListModel<Object> getDcModel() {
        if (!(getModel() instanceof DcListModel))
            setModel(new DcListModel<Object>());
        
        return (DcListModel<Object>) getModel();
    }
