...
public class ImdbPerson {
    ...
    private static Logger logger = DcLogManager.getLogger(ImdbPerson.class.getName());
    ...
    public DcObject get(String id) throws Exception {
        URL url = new URL(address + "/name/" + (id.startsWith("nm") ? "" : "nm") + id);
        return get(url);
    }    
    ...
    public DcObject get(URL url) throws Exception {
        ...
        name = name.indexOf("- IMDb") > -1 ? name.substring(0, name.indexOf("- IMDb")) : name;
    ...
    private void setBiography(DcObject person, String id) throws Exception {
        /*String imdbUrl = "http://www.imdb.com/name/nm" + id + "/bio";
        try {
            Document document = HtmlUtils.getDocument(new URL(imdbUrl), StandardCharsets.ISO_8859_1);
            ...
            logger.debug("Could not parse information from " + imdbUrl);
        } */
    }
    ...
    private void setPhoto(DcObject person, Document document) throws Exception {
        ...
        if (node != null) {
            ...
            if (!url.contains("nophoto.jpg")) {
                DcImageIcon icon = new DcImageIcon(HttpConnectionUtil.retrieveBytes(url));
                person.setValue(DcAssociate._D_PHOTO, icon);
            }
        } else {
            node = (Node) xpath.evaluate("//td[@id='img_primary']//img", document, XPathConstants.NODE);
            if (node != null) {
                String url =node.getAttributes().getNamedItem("src").getTextContent();
                if (!url.contains("nophoto.jpg")) {
                    DcImageIcon icon = new DcImageIcon(HttpConnectionUtil.retrieveBytes(url));
